import { addMonths, dateInMonth, monthStart } from "@uniquedev/haven-shared";
...
import { seededRng } from "../lib/deterministic-rng.js";
...
async function upsertRecurring(entry: RecurringSeed): Promise<void> {
  ...
  if (existing) {
    ...
    await db.insert(recurringEntries).values(values);
  }
}
...
async function upsertPurchase(entry: PurchaseSeed): Promise<void> {
  ...
  if (existing) {
    ...
    await db.insert(plannedPurchases).values(values);
  }
}
